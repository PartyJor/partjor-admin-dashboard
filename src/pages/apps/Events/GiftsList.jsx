...
function ReactTable({ data = [], columns }) {
  ...
  return (
    <>
      <Typography style={{ marginBottom: 15, fontSize: 20, fontWeight: 'bold' }}>Gifts List for {title}</Typography>
...
export default function GiftsListPage() {
    ...
    const handleFetchEventDetails = async () => {
      if (lists) {
        ...
        if (eventDetails && eventDetails.included) {
          const allInvitations = eventDetails.included.filter((detail) => detail.type === 'gifts') || [];
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Is Committed to',
        accessorKey: 'attributes.is_committed_to',
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <a href={getValue()} target="_blank" rel="noopener noreferrer" style={{ color: '#ff9a30' }}>
            {' '}
            <Link to={getValue()} style={{ color: 'inherit' }}>
              {' '}
              View Item{' '}
            </Link>
          </a>
